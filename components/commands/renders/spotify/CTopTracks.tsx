...
import { AnimatedSpan, useTypewriter } from "@/components/AnimatedComponents";
import { useGrep, useGrepRaw } from "@/components/providers/PipelineProvider";
import { computeTypeLineMs, EASE_OUT } from "@/lib/animation/motion";
import { filterByGrep } from "@/lib/utils/grep.utils";
import SpotifyQuery from "./SpotifyQuery";

type TopTracksData = NonNullable<Awaited<ReturnType<typeof getTopTracks>>>;

function TopTracksTable({ data }: { data: TopTracksData }) {
  const t = useTranslations("Spotify");
  const grep = useGrep();
  const grepRaw = useGrepRaw();

  const ranked = data.items.map((item, index) => ({ item, rank: index + 1 }));
  ...
  // Reveal rows one at a time, like typing.
  const count = useTypewriter(
    filtered.length,
    computeTypeLineMs(filtered.length),
  );
  ...
  return (
    <AnimatedSpan>
      <table className="w-full max-w-full table-fixed">
        ...
        <tbody>
          {filtered.slice(0, count).map(({ item, rank }) => (
            <m.tr
              key={item.id}
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
            ...
            >
              <td className="text-left align-middle tabular-nums">{rank}</td>
