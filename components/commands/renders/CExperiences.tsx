...
function CExperiences() {
  ...
  const visible = EXPERIENCES.filter((experience) => {
    if (!grep) return true;
    const period = formatPeriod(experience, format, tPeriod);
  ...
  return (
    <TimelineTypewriter
      entries={visible.map((experience) => {
        const period = formatPeriod(experience, format, tPeriod);
        ...
        return {
          key: experience.slug,
          marker: experience.end ? undefined : (
            <div className="absolute inset-s-[-5px] top-1 size-2 animate-ping rounded-full bg-primary opacity-75" />
          ),
          lines: [
