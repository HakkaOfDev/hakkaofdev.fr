...
function CHelp() {
  ...
  const filteredGroups = useMemo(() => {
    if (!grep) return GROUPED_COMMANDS;
    return GROUPED_COMMANDS.map(({ meta, commands }) => ({
      meta,
      commands: commands.filter((cmd) => {
        const description = tCommands(
          `descriptions.${cmd.slug}` as never,
        ) as string;
        return (
          cmd.command.toLowerCase().includes(grep) ||
          description.toLowerCase().includes(grep)
        );
      }),
    ...
    <AnimatedSpan className="gap-4">
      ...
      {filteredGroups.length === 0 ? (
        <p className="text-muted-foreground text-xs">
          {tCommands("help.noMatches", { pattern: grepRaw })}
        </p>
      ) : (
        <div className="grid grid-cols-[max-content_1fr] items-center gap-x-3 gap-y-1.5">
          {filteredGroups.map(({ meta, commands }, idx) => (
          ...
          {!grep && (
            <>
              <SectionHeading
                title={tTerminal("shortcutsHeading")}
                withTopSpacing
              />
